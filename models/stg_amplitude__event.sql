...
final as (
    ...
    and cast({{ dbt.date_trunc('day', 'event_time') }} as date) <= {{ "cast('" ~ var('date_range_end', dbt.dateadd("month", 1, dbt.date_trunc("day", dbt.current_timestamp()))  ) ~ "' as date)" }} -- filter to records before a specific date 
